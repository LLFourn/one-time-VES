...
In 2017, Andrew Poelstra posted a message to the Mimblewimble mailing list\cite{poelstra-bitcoin-dev-scriptless} demonstrating an interesting feature of the Schnorr signature scheme. He showed that, through a small tweak to the signing algorithm the core lock construction of the lightning payment channel network\cite{poon2016bitcoin} could be realised without using Bitcoin's \emph{smart contract} language called \emph{script}. This was a remarkable discovery as the existing proposal relied heavily on enforcing the lock logic using script based hash locks. His explanation ended with ``I'm very excited about this'' and in retrospect his excitement seems justified. This breakthrough idea continues to be used in interesting ways to create scriptless variants of major Bitcoin layer-2\footnote{In this work we will use layer-2 to refer to any protocol that requires a direct communication channel between participants not just "off-chain" payment channel type protocols} protocol proposals (we give a fairly comprehensive list in Section~\ref{exisitng-protocols}).

\textbf{Scriptless scripts.} Achieving smart contract logic without script has been termed \emph{scriptless scripts} within the Bitcoin research community. It is a significant challenge to the standard conception of smart contracts. Instead of being expressed in a programming language, a smart contract becomes a kind of multi-party computation that outputs transaction signatures according to the desired contract logic. These signatures are indistinguishable from standard signatures on normal payment transactions. This is compelling because the contract rules are not revealed explicitly on the blockchain, reducing what can be learned by a passive observer.

\textbf{Verifiably Encrypted Signatures.} In our work we frame the adaptor signature, the key building block of scriptless protocols, as a kind of signature encryption. In particular, we find it almost fits the definition of \emph{verifiably encrypted signatures}\cite{Boneh:2003:AVE:1766171.1766207} (VES) introduced for the BLS signature scheme\cite{Boneh:2001:SSW:647097.717005}. A VES scheme enables a signer to produce an encrypted signature where the validity can be verified non-interactively. The only difference between a VES and an adaptor signature is that the encryption is \emph{one-time} in a similar sense to a one-time pad: the decryption key can easily be recovered from the ciphertext and its plaintext (the signature).

This ``one-time'' property may seem like a bug, but in many settings it is actually a powerful feature. Consider the motivating example for the original VES scheme: Two parties, Alice and Bob, wish to fairly exchange signatures with the assistance of a trusted third party named the \emph{adjudicator}. First, Alice sends to Bob her signature encrypted under the adjudicator's public key. Bob verifies the validity of the signature encryption and then sends his signature to Alice. If Alice refuses to reciprocate, Bob may ask the adjudicator to intervene by decrypting his encryption of Alice's signature. If we assume the adjudicator is able to correctly assess the situation (i.e.\ will not be tricked by a malicious Bob) then this a secure optimistic fair signature exchange protocol.

With a one-time VES we can achieve the fair exchange of signatures \emph{without} a trusted party as long a the signatures need to be publicly available to be useful (e.g.\ transaction signatures on a public ledger). The one-time property gives the protocol designer leverage over malicious parties by forcing them to leak us to let the one of the possibly malicious participants generate the signature encryption key-pair.


Instead of a trusted party having the decryption key, we let Alice generate the encryption key-pair. Along with her encrypted signature, Alice sends the public encryption key to Bob. Bob then encrypts his signature to Alice under the same key. Alice decrypts Bob's signature and posts it to the public ledger. The one-time property of the VES ensures that Bob, upon seeing Alice's decryption of his signature, will be able to recover the decryption key and decrypt his encryption of Alice's signature. Thus, without a trusted party we can achieve the fair exchange of signatures under the assumption that Alice will have publicly post the decrypted signature in order to benefit from it.

\subsection{Survey of Scriptless Protocols}
\label{exisitng-protocols}

In order to motivate our formalisation, we first discuss how adaptor signatures are used in existing scriptless proposals. We give a simplified explanation of each protocol below, focusing on the role the VES plays in the construction. As we will discuss in Section~\ref{semi-scriptless}, each scheme requires a VES scheme with a two-party encrypted signing algorithm but we leave this out of our description for clarity.  With the exception of the ``Discreet Log Contracts'' and lottery protocols, they each require a one-time VES\@. In general, each scriptless protocol is more efficient in terms of transaction footprint and more confidential than its script based counterpart.

\hfill \break \textbf{Atomic swaps.}  An atomic swap enables two parties, Alice and Bob, to exchange the ownership of two assets, $A$ and $B$, on two different ledgers,  $\alpha$ and $\beta$. As initially conceived\cite{atomic-swap}, Alice generates a random secret $y$ and shares the hash of it $Y \gets H(y)$ with Bob. Alice then locks $A$ in a smart contract on $\alpha$ that will only release $A$ if the Bob activates it with $y'$ such that $Y = H(y')$. Bob locks $B$ into a similar contract on $\beta$. Alice then claims $B$, by activating the contract on $\beta$ with $y$. Bob then learns $y$ and claims $A$ with it on $\alpha$.
...
\hfill \break  \textbf{Pay for commitment.} A script based smart contract to pay for an opening hash commitment is straightforward to construct. A scriptless alternative for paying for the opening of a Pedersen commitment has been proposed in \cite{pay-for-pedersen}.

\subsection{Our Contribution}

In Section~\ref{VES-section}, we contribute to theory of verifiably encrypted signatures in general. The original security definitions\cite{Boneh:2003:AVE:1766171.1766207} only required the scheme to be secure if the encryption key-pair was chosen by the trusted adjudicator. VES schemes secure by these definitions are generally inappropriate for use in layer-2 protocols where an interactive trusted party is rarely assumed. We propose new stronger security definitions without reference to trusted parties to address this and show that all existing schemes satisfy our new definitions.
...
As far as we are aware, this work represents the first attempt to formalise the adaptor signature as a primitive on its own. However, the works of \cite{cryptoeprint:2018:472} and \cite{cryptoeprint:2019:589} use the idea of the adaptor signature and formally argue their security in the \emph{Universal Composability} (UC) model\cite{UC}. Their UC simulation ensures that corrupted parties may learn nothing but the signature itself from any adaptor signatures they receive. In general, the UC simulation is achieved through efficient \emph{non-interactive zero knowledge proofs of knowledge} for discrete logarithms. This allows the simulator in the ideal world to extract the secret keys of the corrupted party and use them to synthetically create adaptor signatures to simulate the view of the adversary in the real world.
...
\EUFCMAVES effectively replaces the original definition of \emph{unforgeability} so we now discuss and prove the relationship between the two. The original unforgeability property referred to signature encryptions being unforgeable under the trusted adjudicator's encryption key. Our definition says nothing about the unforgeability of signature encryptions. In fact, an adversary who can produce valid VES ciphertexts without the secret signing key is perfectly compatible with our definition. Of course, they will never be able to forge a VES ciphertext under a particular encryption key. If they could do that then they could trivially forge an encrypted signature under a key for which they know the decryption key and decrypt it. The original definitions seem to have missed this intuition: it is not the security of the VES scheme that prevents there being a successful forger of signature encryptions under a particular key, the \EUFCMA security of the underlying signature scheme already ensures that no such algorithm can exist.
We use this intuition to prove that any \EUFCMAVES scheme is also unforgeable. First we recall the original definition.
...
Now we show that any forger who breaks unforgeability breaks \EUFCMAVES, which implies that \EUFCMAVES is at least as strong as unforgeability.

\begin{theorem}[\EUFCMAVES $\implies$ Unforgeability]
If a VES scheme is \EUFCMAVES secure and has the \emph{validity} property then it also has the original BGLS \emph{unforgeability} property defined above.
\end{theorem}
...
\begin{figure}[h]
    ...
    \end{pchstack}
    \end{mdframed}
    \caption{The algorithms of the ECDSA adaptor signature scheme. $f: \G \rightarrow \ZZ_q$ converts a an elliptic curve point to its x-coordinate mod $q$.}
